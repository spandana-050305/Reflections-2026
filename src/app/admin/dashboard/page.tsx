...
export default async function AdminDashboard() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Final Year Dashboard</h2>
        <p className="text-gray-500 mt-1">Reflections administration</p>
      </div>

      {/* Registration Status */}
      <div className={`rounded-xl p-4 flex items-center justify-between ${settings?.registration_open ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
        ...
        <a href="/admin/settings" className="text-sm text-gray-500 hover:text-brand-600 underline">Change →</a>
      </div>

      {/* Stats — Events removed */}
      <div className="grid grid-cols-2 gap-4">
        <div className="card text-center">
          <p className="text-3xl font-bold text-brand-600">{totalSchools}</p>
          <p className="text-sm text-gray-500 mt-1">Schools</p>
        </div>
        <div className="card text-center">
          <p className="text-3xl font-bold text-brand-600">
            {Object.values(filledMap).filter(s => s.size > 0).length}
          </p>
