...
export async function middleware(request: NextRequest) {
  ...
  if (!user && !publicRoutes.some(r => pathname === r || pathname.startsWith('/results'))) {
    const loginUrl = pathname.startsWith('/school') ? '/school/login' : '/login'
    return NextResponse.redirect(new URL(loginUrl, request.url))
  }

  // Already logged in → skip login/landing page, go to dashboard
  if (user && (pathname === '/' || pathname === '/login' || pathname === '/school/login')) {
  ...
  if (user) {
    if (pathname.startsWith('/school')      && role !== 'school')                                          return NextResponse.redirect(new URL('/school/login', request.url))
